...
`Expires`是**服务器响应消息头字段**。缓存到期时间，用来指定资源到期时间，也即`Expires =max-age+请求时间`，需要和`Last-modified`结合使用。在响应http请求时告诉浏览器在过期时间前浏览器可以直接从浏览器缓存获取数据，而无需再次请求。
...
强缓存判断缓存是否超过某个时间或者某个时间段，而不关心服务器端文件是否已经更新，这可能会导致加载文件不是服务器端最新的内容（因此有时候需要刷新浏览器缓存就是这个道理），下面介绍一下协商缓存：
